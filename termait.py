...
def process_request(prompt: str, provider: LLMProvider):
    ...
    def _(event):
        "Bind Esc to exit the prompt with None"
    ...
    while True:
        try:
            with console.status(f"[bold green]Generating command using {provider.name}...", spinner="dots"):
                content = provider.generate_command(current_prompt)
            ...
            console.print()
            console.print("[dim]Press [bold]Enter[/bold] to run, [bold]Esc[/bold] to cancel, or type to refine options.[/dim]")
            
            # Interactive prompt loop
            user_input = session.prompt("(Run) > ")
            ...
            if user_input.strip() == "":
                ...
                # Refine
                console.print(f"[blue]Refining request: {user_input}[/blue]")
                # Update the prompt with history/context
                # We append the refinement to the conversation context implicitly by updating the prompt
                current_prompt = f"{current_prompt}\nConstraint/Refinement: {user_input}"
                continue
